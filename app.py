...
anio_seleccionado = st.selectbox("Año:", ["2024", "2025"])

# Cargar los datos correspondientes al año seleccionado
if anio_seleccionado in urls:
    ...
    if not data_reemplazos.empty and not data_clases_totales.empty:
        ...
        if grafico_seleccionado == "Reemplazos por Mes":
            ...
            fig.update_layout(
                title=f"Reemplazos Atendidos por Mes en {anio_seleccionado}",
                xaxis_title="Mes",
            ...
            fig = px.bar(
                ...
                y="CANTIDAD",
                text="CANTIDAD",
                title=f"Reemplazos por Programa en {anio_seleccionado}",
            )
            ...
            fig = px.bar(
                ...
                y="CANTIDAD",
                text="CANTIDAD",
                title=f"Reemplazos por Motivo en {anio_seleccionado}",
            )
            fig.update_traces(textposition="auto")
            fig.update_layout(xaxis_title="Motivo de Reemplazo", yaxis_title="Cantidad", template="simple_white")
            st.plotly_chart(fig, use_container_width=True)
